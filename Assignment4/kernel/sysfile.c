...
sys_exec(void)
{
  ...
  struct inode *ip = namei(path);
   ...
   if(ip->type == T_SYMLINK)
   {
       ...
       {
         if (ip->type == T_SYMLINK) //another check for the new ip
         {
           int output = readi(ip, 0, (uint64)path, 0, MAXPATH);
           if(output <= 0)
           {
              iunlock(ip);
              end_op();
...
sys_symlink(void)
{
  ...
  int len = writei(ip, 0 ,(uint64)oldpath, 0,  MAXPATH);
  ...
  sys_readlink(void)
  {
    ...
    int output = readi(ip, 0 , (uint64)buff, 0,bufsize);

    struct proc *p;
    p = myproc();

    if(copyout(p->pagetable, addr, buff, bufsize) < 0){
      iunlock(ip);
      end_op();
      return -1;
    }
